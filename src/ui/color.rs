...
fn make_picker_row(
    ...
    let mut vinput = ValueInput::new(
        PICKER_LABEL_WIDTH + PICKER_SLIDER_WIDTH, ypos,
        PICKER_INPUT_WIDTH, PICKER_ROW_HEIGHT, None);
...
pub fn pick_color(start: RGB) -> Option<RGB> {
    ...
    let (_, _, _) = make_picker_row(
        0, "R", start.r() as f64, prev.clone(), rvalue.clone()
    );
    let (_, _, _) = make_picker_row(
        PICKER_ROW_HEIGHT, "G", start.g() as f64, prev.clone(), rvalue.clone()
    );
    let (_, _, _) = make_picker_row(
        2 * PICKER_ROW_HEIGHT, "B", start.b() as f64, prev.clone(), rvalue.clone()
    );
...
impl ColorPaneGuts {
    fn new(
        ...
        let pg = Rc::new(RefCell::new(ColorPaneGuts {
            choosers: new_gradients.iter()
                .map(|g| GradientChooser::new(*g)).collect(),
            win: w.clone(),
    ...
    fn redraw(&mut self) {
        ...
        let _ = Frame::default().with_label("Color Map")
            .with_pos(0, 0).with_size(COLOR_PANE_WIDTH, GRADIENT_ROW_HEIGHT);
            
        for (n, ch) in self.choosers.iter_mut().enumerate() {
            let ypos = (1 + n as i32) * GRADIENT_ROW_HEIGHT;
            let mut insert_butt = Button::default().with_label("@+")
                .with_size(GRADIENT_BUTTON_WIDTH, GRADIENT_ROW_HEIGHT)
                .with_pos(0, ypos);
            ...
            //ch.show();
            let mut remove_butt = Button::default().with_label("x")
                .with_size(GRADIENT_BUTTON_WIDTH, GRADIENT_ROW_HEIGHT)
            ...
            insert_butt.set_callback({
                ...
                move |_| { me.borrow_mut().insert(n); }
            });
            
            remove_butt.set_callback({
                let me = self.me.as_ref().unwrap().clone();
                move |_| { me.borrow_mut().remove(n); }
            });
        }
        ...
        for ch in self.choosers.iter_mut() { ch.show(); }
            
        append_butt.set_callback({
            let me = self.me.as_ref().unwrap().clone();
            move |_| { me.borrow_mut().append(); }
        });
    ...
    fn insert(&mut self, n: usize) {
        ...
        let g = Gradient { start: new_start, end: new_end, steps: 256 };
...
mod test {
    ...
    fn query_gradient() {
        let a = fltk::app::App::default();
        let g = Gradient { start: RGB::BLACK, end: RGB::WHITE, steps: 256 };
