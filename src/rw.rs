...
pub fn save_with_metadata<P: AsRef<Path>>(
    ...
    let f = match File::create(fname) {
        Ok(f) => f,
        Err(e) => {
            let estr = format!("Error opening {} for writing: {}",
                                fname.display(), &e);
            return Err(estr);
        },
    };
    ...
    if let Err(e) = enc.add_itxt_chunk("jset_desk parameters".to_string(),
                                        metadata) {
...
fn try_to_fill<R: Read>(r: &mut R, buff: &mut [u8])
-> Result<usize, std::io::Error> {
    ...
    loop {
        match r.read(&mut buff[total_read..])? {
            ...
            n => { total_read =  total_read + n; }
        }
    }
}

fn try_load_toml(f: &mut File) -> LoadResult {
    let mut buff: Vec<u8> = Vec::with_capacity(READ_LIMIT);
    buff.resize(READ_LIMIT, 0u8);
    
    let str_len = match try_to_fill(f, &mut buff) {
        Ok(n) => n,
        Err(e) => { return LoadResult::GiveUp(e.to_string()); }
    };
    
    let toml_str = match std::str::from_utf8(&buff[..str_len]) {
        Ok(s) => s,
        Err(_) => { return LoadResult::TryOtherType; },
    };
...
fn try_load_png(f: &mut File) -> LoadResult {
    ...
    for chunk in rdr.info().utf8_text.iter() {
        if &chunk.keyword == "jset_desk parameters" {
            match chunk.get_text() {
                ...
                Err(e) => { eprintln!("Error decoding metadata text chunk: {}", &e); },
            }
        }
    }
    ...
    let meta_text = match meta_text {
        Some(s) => s,
        None => { return LoadResult::GiveUp("File contains no recognizable metadata parameters.".to_string()); },
    };
...
-> Result<(ImageDims, ColorSpec, IterType), String> {
    ...
    match try_load_toml(&mut f) {
        ...
        LoadResult::GiveUp(e) => { return Err(e); },
        LoadResult::TryOtherType => { /* continue trying other type! */ },
    }
    
    if let Err(e) = f.seek(std::io::SeekFrom::Start(0)) { return Err(e.to_string()); }
